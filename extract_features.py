...
def load_rgb_batch(frames_dir, rgb_files, frame_indices):
    batch_data = np.zeros(frame_indices.shape + (256, 340, 3))  # 变成 batch_size, chunk_size, 256, 340, 3
...
def oversample_data(data):
    data_flip = np.array(data[:, :, :, ::-1, :])  # 竖直反转
...
def run(i3d, frequency, frames_dir, batch_size, sample_mode):
    ...
    rgb_files = [i for i in os.listdir(frames_dir)]  # 视频转换成的图片
    rgb_files.sort()
    ...
    for i in range(clipped_length // frequency + 1):
        frame_indices.append([j for j in range(i * frequency, i * frequency + chunk_size)])  # chunk_size=16
    frame_indices = np.array(frame_indices)
    chunk_num = frame_indices.shape[0]
    batch_num = int(np.ceil(chunk_num / batch_size))  # Chunks to batches
    frame_indices = np.array_split(frame_indices, batch_num, axis=0)  # 进行分割，允许数量不均等，一共batch_num个
